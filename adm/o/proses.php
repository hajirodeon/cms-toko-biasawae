...
if ($_POST['btnKRMx'])
	...
	for ($i=1; $i<=$limit;$i++)
		...
		$yuk = "noresi";
		$yuhu = "$yuk$i";
		$noresi = cegah($_POST["$yuhu"]);
		
		
		$yuk = "e_tglkirim";
		$yuhu = "$yuk$i";
		$e_tglkirim = balikin($_POST["$yuhu"]);
		$tglku = explode("/", $e_tglkirim);
		$e_tgl = trim($tglku[0]);
		$e_bulan = trim($tglku[1]);
		$e_tahun = trim($tglku[2]);
		$e_tglkirim = "$e_tahun:$e_bulan:$e_tgl";
		


		mysqli_query($koneksi, "UPDATE member_order SET tgl_kirim = '$e_tglkirim', ".
						"jasakirim_noresi = '$noresi' ".
						"WHERE kd = '$kd'");
		}
...
if (empty($s))
	...
	if ($count != 0)
		...
		<div class="table-responsive">          
		  <table class="table" border="1">
		    <thead>
				...
				<th>KONFIRMASI PROSES BUNGKUS</th>
				<th>TGL BOOKING</th>
		        <th>MEMBER</th>
		        <th>NOTA KODE</th>
		        <th>ITEM</th>
		...
		do
			...
			$nomer = $nomer + 1;
			$kd = nosql($data['kd']);
			$i_noresi = balikin($data['jasakirim_noresi']);
			...
			$(document).ready(function(){
			    ...
			    $('#e_tglkirim<?php echo $nomer;?>').datepicker({
			        format: 'dd/mm/yyyy',
			        todayHighlight: true,
			        autoclose: true,
			    })
			...
			No. Resi Kirim :
			<br> 			
			<input name="kdku'.$nomer.'" id="kdku'.$nomer.'" type="hidden" value="'.$kd.'">
			<input name="noresi'.$nomer.'" id="noresi'.$nomer.'" type="text" width="10" value="'.$i_noresi.'" class="btn btn-info">
			</p>
			
			<p>
			Tanggal Kirim :
			<br>
			<input name="e_tglkirim'.$nomer.'" id="e_tglkirim'.$nomer.'" type="text" size="10" value="'.$e_tglkirim.'" class="btn btn-info">
