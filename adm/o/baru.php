...
if ($s == "proses")
	...
	mysqli_query($koneksi, "UPDATE member_order SET tgl_proses = '$tglproses' ".
					"WHERE kd = '$kd'");
...
if ($s == "tolak")
	{
	//update
	mysqli_query($koneksi, "UPDATE member_order SET tgl_proses = '0000-00-00', ".
...
if (empty($s))
	...
	if ($count != 0)
		...
		<div class="table-responsive">          
		  <table class="table" border="1">
		    <thead>
				...
				<th>KONFIRMASI TRANSFER</th>
				<th>PRINT ALAMAT KIRIM</th>
				<th>TGL BOOKING</th>
		        <th>MEMBER</th>
		        <th>NOTA KODE</th>
		        <th>ITEM</th>
		...
		do
			...
			Bukti Transfer :
			<br>
			<a href="'.$sumber.'/filebox/konfirmasi/'.$i_filex_bayar.'" target="_blank">
			<img src="'.$sumber.'/filebox/konfirmasi/'.$i_filex_bayar.'" width="100">
			</a>				
			...
			<br>
			[<a href="'.$filenya.'?s=proses&kd='.$kd.'">CEK DAN PROSES</a>].
			<br>
			<br>
			<br>
			[<a href="'.$filenya.'?s=tolak&kd='.$kd.'">TOLAK</a>].
			</td>
			<td>
			[<a href="'.$filenyax.'?kd='.$kd.'" target="_blank">PRINT</a>].
			</td>
			<td>'.$i_booking.'</td>
			<td>'.$i_member_nama.'</td>
			<td>'.$i_nota.'</td>
