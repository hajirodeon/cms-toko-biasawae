...
if ($s == "proses")
	...
	$yuk_kabupaten = balikin($ryuk['penerima_kabupaten']);
	$yuk_propinsi = balikin($ryuk['penerima_propinsi']);
	
	$e_alamat = "$yuk_alamat, $yuk_kelurahan, $yuk_kecamatan, $yuk_kabupaten, $yuk_propinsi";	
...
if (empty($s))
	...
	if ($count != 0)
		...
		do
			...
			<br>
			[<a href="'.$filenya.'?s=proses&kd='.$kd.'">PAKET TELAH DITERIMA</a>].
			<td>'.$i_tgl_proses.'</td>
			<td>'.$i_tgl_booking.'</td>
			<td>'.$i_member_nama.'</td>
			<td>'.$i_nota.'</td>
