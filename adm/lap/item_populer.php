...
//nilai
$filenya = "item_populer.php";
$judul = "[LAPORAN] Item Populer";
$judulku = "$judul";
$judulx = $judul;
...
if (empty($s))
	...
	if (!empty($kunci))
		...
		$sqlcount = "SELECT * FROM m_item ".
						...
						"AND (nama LIKE '%$kunci%' ".
						"OR isi LIKE '%$kunci%' ".
						"OR jml_dilihat LIKE '%$kunci%') ".
						"ORDER BY round(jml_dilihat) DESC";
		$sqlresult = $sqlcount;
	...
	if ($count != 0)
		...
		do
			...
			$i_item_nama = balikin($data['nama']);
			$i_postdate = balikin($data['postdate']);

			echo "<tr valign=\"top\" bgcolor=\"$warna\" onmouseover=\"this.bgColor='$warnaover';\" onmouseout=\"this.bgColor='$warna';\">";
			echo '<td width="50">'.$i_jml_dilihat.'</td>
			<td>'.$i_item_nama.'</td>
	    	</tr>';
			}
